...
function checkTxnid($txnid) {
    ...
    $results = $db->query('SELECT * FROM `payment_detail` WHERE txnid = \'' . $txnid . '\'');
...
function addPayment($data) {
	global $db;
	 ...
	 if (is_array($data)) {
		$stmt = $db->prepare('INSERT INTO `payment_detail` (txnid, ICno, payment_amount, itemid, payment_method, createdtime) VALUES( ?, ?, ?, ?, ? )');
		$stmt->bind_param(
			'sdsss',
            $data['txn_id'],
			$data['ICno'],
...
class PaymentPage{
		...
		public function paymentcheck()
	{
		...
		$query = "SELECT cardnum, holdername FROM payment";
